...
| `$finish-chapter` 또는 `이번 챕터 정리해줘` | 실습 보관·챕터 회고·knowledge 갱신·검증 후 초기화, STATE 승인 후 로컬 커밋 |
...
챕터 마무리 요청은 아래 범위의 로컬 commit까지 포함하는 예외입니다.
...
챕터가 끝나면 [`$finish-chapter`](./.agents/skills/finish-chapter/SKILL.md)를
호출합니다. 저장된 노트북을 그대로 보관하고, 옆의 챕터 회고에 과정·도움·
결과·한계를 정리하며, 재사용할 개념은 knowledge에 반영합니다. 보관 검증과
문서 검사, 학습자의 무보조 knowledge 초안 확인과 교정이 끝나면 `main.ipynb`를
초기화합니다. 초안이 없으면 먼저 요청하며 AI가 대신 작성하지 않습니다.
STATE 전체 교체안을 승인하면
