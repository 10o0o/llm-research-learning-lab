...
샘플링은 확률에 따라 후보를 뽑고, greedy는 최대 확률 후보를 고른다.
같은 모델과 시작 문맥에서도 샘플링은 다른 경로를 갈 수 있다. 고정된 모델의
greedy는 난수를 사용하지 않아 샘플링용 seed의 영향을 받지 않는다.
...
같은 bigram 가중치와 시작 문자에서 한 샘플링 결과는 서로 다른 이름 다섯 개였고,
greedy 결과는 `. → a → .` 경로의 `a` 다섯 개였다. 이 결과에서는 모두 종료
문자에 도달했다. 샘플링이 항상 서로 다른 이름을 만들거나 항상 종료 문자에
도달한다는 보장은 없다.
