...
이 문서는 장기 참고 catalog이며 현재 학습 범위를 정하지 않는다. 이번 경로에서 뺀 자료와 복귀 조건은 `DEFERRED.md`에 있고, 실제 수행 순서와 시간 예산은 `ROADMAP.md`의 Phase 표에 있다. 아래 표에 연결된 자료 중 상당수는 현재 경로 밖이며, 그 사실이 역량 자체의 취소를 뜻하지는 않는다.
