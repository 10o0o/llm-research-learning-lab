...
이 문서는 장기 참고 catalog이며 현재 학습 범위를 정하지 않는다. 이번 경로에서 뺀 자료와 복귀 조건은 `DEFERRED.md`에 있고, 실제 수행 순서와 시간 예산은 `ROADMAP.md`의 Phase 표에 있다. `CC-STAT-01`·`CC-STAT-02`처럼 충족도가 `없음`인 역량은 `P0`에서, `CC-PROB-02`·`CC-PROB-03`·`CC-ML-02`의 `부분`은 `P0`~`P1`에서 메운다. `SRC-KAM-*`와 `SRC-KDL-*`는 이제 대조용이 아니라 `P1`·`P2`의 정식 주자료다.
...
아래 역량·선수관계·필수 module·milestone 표는 장기 목표의 참고 catalog다.
승인된 주강의 순서를 강제 변경하거나 모든 선행 산출물의 완료를 현재
강의 진입 조건으로 요구하지 않는다. 실제 학습은 `ROADMAP.md`의 승인된
주강의 경로와 `STATE.md`의 현재 범위를 따르며, 현재 이해에 직접 필요한
선수개념을 보강한 뒤 같은 주강의로 돌아간다. 기존 자료 기록과 catalog는
장기 참고로 보존한다.
