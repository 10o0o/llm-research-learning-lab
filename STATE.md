...
- 현재 강의: Chapter 2, §2.8.1 Affine Subspaces, 인쇄 쪽 61~62
- 공식 자료: https://mml-book.github.io/
- 공식 PDF: https://mml-book.github.io/book/mml-book.pdf
- 연결 실습: Example 2.26과 비동차 연립방정식의 해집합 연결; Exercise 2.20(a)의 그림은 미확인
...
Exercise 2.20(a)는 원점에서만 만난다는 설명을, 서로 실수배 관계가 아닌
두 벡터라는 판별로 교정했다. 그림 작성 여부는 확인하지 않았다.
Exercise 2.20(b)는 좌표 변환행렬의 의미를 설명받은 뒤 두 열의 계수를
정확히 제시했다. 행렬의 열 배치는 튜터 피드백으로 연결했다.
Exercise 2.20(c)는 세 피벗으로 기저임을 판단했다. 좌표 변환행렬에서는
기저벡터를 행으로 배치해, 열에 배치하는 이유와 결과를 튜터가 교정했다.
Exercise 2.20(d)는 두 식을 더하고 빼는 안내 후 각 기저벡터의 상을
출력 기저로 표현한 두 좌표 열을 정확히 구했다.
Exercise 2.20(e)는 합성 행렬의 마지막 원소를 피드백으로 교정했다.
입력 좌표 변환과 선형변환의 연결을 어려워해 대입 관계로 설명했다.
(f)는 주어진 행렬과 계산 순서에 따라 네 좌표 계산을 모두 정확히 수행했고,
단계별 계산과 합성 행렬의 결과가 일치했다. 개념의 독립 설명은 별도 확인하지 않았다.
기저 변환의 연결에 대한 추가 질문 후, 사용자의 요청으로 §2.7.3으로 이동했다.
§2.7.2 일반식의 출력 역변환 방향 설명은 남아 있다.
§2.7.3에서는 상과 영공간을 혼동해 입력·출력 예시로 보완했다.
Exercise 2.19(a)의 영공간과 그 차원은 정확히 구했으나, 상도 0차원이라는
판단은 피드백으로 교정했다. 영공간과 단사의 관계는 설명 후 이해했다고 응답했다.
단사·전사 관계와 아핀 부분공간의 도입은 설명 후 이해했다고 응답했다.
현재는 부분공간의 평행이동을 연립방정식의 해집합과 연결하는 단계다.
...
다음은 §2.8.1에서 해 하나에 영공간을 더해 전체 해집합을 표현하는 이유와
그 차원을 연결하는 것이다. 이후 §2.8.2 아핀 사상으로 이어간다.
