...
MML §2.4.3의 부분공간 조건을 설명·읽기로 연결하고 Exercise 2.9(a)를
학습자가 직접 판정한다. 집합이 영벡터를 포함하고 덧셈·실수배에 대해
닫혀 있는지 이유를 설명한다. 이어서 같은 공식 문제의 (b)~(d)를 다룬다.
코드 실행·환경 설치·GPU 대여는 필요 없다. 아직 문제를 시도하거나 통과하지 않았다.
