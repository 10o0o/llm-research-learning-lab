...
새 컴퓨터에 Git, uv, Codex와 Notebook 실행 도구를 준비하고 Codex에 로그인합니다.
...
새 Codex 세션에는 다음처럼 요청합니다.

```text
AGENTS.md와 STATE.md, main.ipynb의 관련 코드와 저장된 출력을 읽고 현재 학습을
이어줘. 새 커널이므로 필요한 셀의 의존관계부터 확인하고, 기존 학습 셀을
무조건 전체 재실행하지 말아줘. 실행은 내가 할게.
```

저장소는 대화 전문이나 Codex 개인 설정·전역 스킬을 전달하지 않습니다.
저장소의 운영 지침과 `.agents/skills/`는 파일로 전달되며 현재 위치는 `STATE.md`로
재개합니다. 인증 정보나 개인 Codex 폴더를 Git에 추가하지 않습니다.
